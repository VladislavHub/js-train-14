...
function nextNewYear() {
  ...
  let currentValue = timeNow.getFullYear();
  
  let nextYear = new Date(currentValue + 1, 0, 1)

  let diff = nextYear - timeNow
...
function addDays(date, days) {
  ...
  return {
    ...
    addedDays: days,
    resultDate: dateNow.toISOString()
  }
};
...
function getAge(birthDate) {
  ...
  let result = dateNow.getFullYear() - birthDate.getFullYear();
...
function compareDates(date1, date2) {
  ...
  if(!typeof date1.getTime() && date2.getTime() === 'function'){
    return "Помилка: вхідне значення має бути об'єктом Date"
  };
...
function getDaysDifference(startDate, endDate) {
  ...
  if(!typeof startDate.getTime() && endDate.getTime() === 'function'){
    return "Помилка: вхідне значення має бути об'єктом Date"
  };
